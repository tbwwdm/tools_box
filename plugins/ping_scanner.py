...
网段 Ping 扫描器 — 独立版
"""
import sys, os, logging, time, subprocess, platform
...
COLOR_MAP = {2: QColor(76, 175, 80), 1: QColor(255, 238, 180), 0: QColor(255, 235, 59)}
...
class PingScanner(QWidget):
    ...
    def _setup_ui(self):
        ...
        legend_items = [(QColor(255, 255, 255), "未扫描"), (QColor(76, 175, 80), "通"), (QColor(255, 235, 59), "不通")]
    ...
    def _update_preview(self):
        ...
        if len(parts) == 3 and all(p.isdigit() and 0 <= int(p) <= 255 for p in parts):
            self.end_preview.setText(f"→ {text}.{self.end_ip.value()}")
        else:
    ...
    def _validate_inputs(self):
        ...
        if len(parts) != 4:
            self.start_ip.setStyleSheet("border: 1px solid #d32f2f; background: #fff5f5;")
            self.start_ip.setToolTip("需要完整的 IPv4 地址")
            return False
        for p in parts:
        ...
        if total > 254:
            self.start_ip.setStyleSheet("border: 1px solid #d32f2f; background: #fff5f5;")
            self.start_ip.setToolTip(f"不能超过 254 个（当前 {total} 个）")
            return False
...
class PingScannerPlugin(PluginBase):
    @property
    def plugin_name(self) -> str: return "网段Ping扫描器"
    @property
    def plugin_name_en(self) -> str: return "Ping Scanner"
    @property
    def plugin_version(self) -> str: return "1.0.0"
    @property
    def plugin_icon(self) -> str: return "🌐"
    @property
    def plugin_description(self) -> str: return "批量 Ping 扫描 IP 网段，可视化显示通断状态"
    @property
    def plugin_description_en(self) -> str: return "Batch ping scan IP ranges, visual grid display"
    @property
    def plugin_tags(self) -> list: return ["network", "ping"]
