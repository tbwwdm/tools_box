...
# ═══════════════════════════════════════════════════════════
#  PluginBase 包装（只显示 YUM 配置标签页）
# ═══════════════════════════════════════════════════════════

from framework.plugin_interface import PluginBase


class YumPlugin(PluginBase):
    @property
    def plugin_name(self) -> str: return "YUM源管理器"
    @property
    def plugin_name_en(self) -> str: return "YUM Repo Manager"
    @property
    def plugin_version(self) -> str: return "1.0.0"
    @property
    def plugin_icon(self) -> str: return "📦"
    @property
    def plugin_description(self) -> str: return "YUM 服务器配置、客户端 .repo 文件分发与检测"
    @property
    def plugin_description_en(self) -> str: return "YUM server config, client .repo distribution"
    @property
    def plugin_tags(self) -> list: return ["yum", "linux"]

    def create_widget(self, parent=None):
        win = MainWindow(lang="zh")
        # 只保留 YUM 标签页（0和1），移除 NTP 和系统初始化
        win.tab_widget.removeTab(3)
        win.tab_widget.removeTab(2)
        return win
